...
def detailTodo(cb=None):
    ...
    tk.Entry(win, state="disabled", textvariable=judul).grid(row=1, column=1, sticky='N')
...
def title():
    waktu = strftime('%H:%M')
    tanggal =str(cal.selection_get())
    root.title(tanggal + " | " +waktu +" | calender ajaib")
    root.after(1000, title)

root = tk.Tk()
s = ttk.Style()
s.configure('Treeview', rowheight=16)
root.title("calender ajaib")

cal = Calendar(root,font='Edo 15',selectmode='day',locale='id_ID',cursor='hand1')
